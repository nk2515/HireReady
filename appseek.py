    ...
    def analyze_job(self, job_description: str) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                reply = response.json()
                parsed_response = json.loads(reply["choices"][0]["message"]["content"])
...
Focus on providing detailed, actionable insights for each field. Keep the JSON structure exact but replace the example content with detailed analysis based on the provided job and resume."""
        ...
        try:
            headers = {
                "Authorization": f"Bearer {self.api_key}",  # DeepSeek API key
                "Content-Type": "application/json",
            }
            # Format the prompt content with job and resume analysis
            ...
            payload = {
                "model": "deepseek-chat",  # You can also try "deepseek-coder" for code-heavy tasks
...
class CoverLetterGenerator:
    ...
    def generate_cover_letter(
        ...
        try:
            ...
            # Format the user prompt with dynamic inputs
            user_prompt = prompt.format(
                job=json.dumps(job_analysis, indent=2),
                resume=json.dumps(resume_analysis, indent=2),
            ...
            payload = {
                "model": "deepseek-chat",  # or "deepseek-coder" if more code-oriented
                "messages": [{"role": "user", "content": user_prompt}],
                "temperature": 0.7,
            }
