...
def generate_updated_resume(resume_text, match_analysis):
    ...
    for line in resume_parts:
        ...
        is_section = line.isupper() or any(
            section in line.upper() for section in common_sections
        )
    ...
    if match_analysis.get("ats_optimization_suggestions"):
        ...
        content.append(
            Paragraph("ATS Optimization Recommendations", section_header_style)
        )
        ...
        for suggestion in match_analysis["ats_optimization_suggestions"]:
            ...
            content.append(
                Paragraph(f"  Suggestion: {suggested}", recommendation_style)
            )
            if keywords:
                content.append(
                    Paragraph(f"  Keywords to Add: {keywords}", recommendation_style)
                )
            if formatting:
                content.append(
                    Paragraph(f"  Formatting: {formatting}", recommendation_style)
                )
            if reason:
...
def generate_updated_resume1(resume_text, match_analysis):
    ...
    for part in resume_parts:
        if part.strip():  # Skip empty lines
            ...
            is_section = part.isupper() or any(
                section in part.upper() for section in common_sections
            )
    ...
    if match_analysis.get("ats_optimization_suggestions"):
        content.append(Spacer(1, 12))
        content.append(
            Paragraph("ATS Optimization Recommendations", styles["Heading2"])
        )
        ...
        for suggestion in match_analysis["ats_optimization_suggestions"]:
            content.append(
                Paragraph(
                    f"• Section: {suggestion['section']}", styles["RecommendationStyle"]
                )
            )
            if suggestion.get("current_content"):
                content.append(
                    Paragraph(
                        f"  Current: {suggestion['current_content']}",
                        styles["RecommendationStyle"],
                    )
                )
            content.append(
                Paragraph(
                    f"  Suggestion: {suggestion['suggested_change']}",
                    styles["RecommendationStyle"],
                )
            )
            if suggestion.get("keywords_to_add"):
                content.append(
                    Paragraph(
                        f"  Keywords to Add: {', '.join(suggestion['keywords_to_add'])}",
                        styles["RecommendationStyle"],
                    )
                )
            if suggestion.get("formatting_suggestion"):
                content.append(
                    Paragraph(
                        f"  Formatting: {suggestion['formatting_suggestion']}",
                        styles["RecommendationStyle"],
                    )
                )
            content.append(Spacer(1, 6))
...
class JobAnalyzer:
    ...
    def analyze_job(self, job_description: str) -> dict:
        ...
        try:
            ...
            parsed_response = json.loads(response.text)  # .text gives the string output
...
Focus on providing detailed, actionable insights for each field. Keep the JSON structure exact but replace the example content with detailed analysis based on the provided job and resume."""
        ...
        try:
            ...
            # Format the prompt with pretty-printed JSON for clarity
            ...
            response = model.generate_content(
                formatted_prompt, generation_config={"temperature": 0.2}
            )

            try:  # Get and clean up the response content
...
class CoverLetterGenerator:
    ...
    def generate_cover_letter(
        ...
        try:
            ...
            response = model.generate_content(
                formatted_prompt, generation_config={"temperature": 0.7}
            )
            return response.text  # Return the full generated cover letter text
...
def main():
    st.set_page_config(
        page_title="LinkedIn Job Application Assistant - HireReady 📝", layout="wide"
    )
    ...
    if job_desc and resume_file:
        with st.spinner("🔍 Analyzing your application..."):
            ...
            if resume_text:
                ...
                match_analysis = job_analyzer.analyze_match(
                    job_analysis, resume_analysis
                )

                if not job_analysis or not resume_analysis or not match_analysis:
                    st.error(
                        "Insufficient data returned from the API. Please try again."
                    )
                    return
                ...
                with tab1:
                    ...
                    matching_skills_count = len(
                        match_analysis.get("matching_skills", [])
                    )
                ...
                with tab3:
                    ...
                    for suggestion in match_analysis.get(
                        ...
                        if suggestion.get("current_content"):
                            st.write(
                                f"**Current Content:** {suggestion['current_content']}"
                            )
                        st.write(
                            f"**Suggested Change:** {suggestion['suggested_change']}"
                        )
                        if suggestion.get("keywords_to_add"):
                            st.write(
                                f"**Keywords to Add:** {', '.join(suggestion['keywords_to_add'])}"
                        ...
                        if suggestion.get("formatting_suggestion"):
                            st.write(
                                f"**Formatting Changes:** {suggestion['formatting_suggestion']}"
                            )
